    ...
    routes: [
      ...
      StatefulShellRoute.indexedStack(
        builder: (context, state, nav) =>
            AppShellScaffold(navigationShell: nav),
        branches: [
          StatefulShellBranch(
            routes: [
              GoRoute(
      ...
      // DONATE FLOW
      GoRoute(
        path: RoutePaths.donateGuidelines,
        name: RouteNames.donateGuidelines,
        builder: (context, state) => const DonateGuidelinesScreen(),
      ),
      GoRoute(
        path: RoutePaths.donateForm,
        name: RouteNames.donateForm,
        builder: (context, state) =>
            DonateFormScreen(imagePath: state.uri.queryParameters['imagePath']),
      ),

      // REQUEST FLOW
      GoRoute(
        path: RoutePaths.requestGuidelines,
        name: RouteNames.requestGuidelines,
        builder: (context, state) => const RequestGuidelinesScreen(),
      ),
      GoRoute(
        path: RoutePaths.requestForm,
        name: RouteNames.requestForm,
        builder: (context, state) {
      ...
      GoRoute(
        path: RoutePaths.categoryMedicines,
        name: RouteNames.categoryMedicines,
        builder: (context, state) => CategoryMedicinesScreen(
      ...
      GoRoute(
        ...
        name: RouteNames.medicineDetails,
        builder: (_, state) => MedicineDetailsScreen(
          medicineId: state.pathParameters['id'] ?? '1',
        ),
      ),
      ...
      GoRoute(
        ...
        builder: (context, state) =>
            const MyActivityListScreen(isDonations: true),
      ),
      GoRoute(
        path: RoutePaths.myRequests,
        name: RouteNames.myRequests,
        builder: (context, state) =>
            const MyActivityListScreen(isDonations: false),
      ),
      ...
      GoRoute(
        ...
        builder: (_, state) => SuccessScreen(
          type: state.uri.queryParameters['type'] ?? 'donation',
        ),
      ),
    ],
  );
      ...
      body: ListView(
        ...
        children: [
          ...
          _QuickActionCard(
            ...
            title: l.donateMedicine,
            subtitle: l.donateSubtitle,
            color: colors.primary,
            onTap: () => context.pushNamed(RouteNames.donateGuidelines),
          ),
          const SizedBox(height: 16),
          _QuickActionCard(
            icon: Icons.medication_rounded,
            title: l.requestMedicine,
            subtitle: l.requestSubtitle,
            color: const Color(0xFF5BC0EB),
            onTap: () => context.pushNamed(RouteNames.requestGuidelines),
          ),
          const SizedBox(height: 16),
          _QuickActionCard(
            icon: Icons.search_rounded,
            title: l.browseAvailable,
            subtitle: l.browseSubtitle,
            color: const Color(0xFF34C759),
            onTap: () => context.goNamed(RouteNames.browse),
          ),
        ],
      ),
    );
